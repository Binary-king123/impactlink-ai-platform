...
// --- CONSTANTS ---
const API_BASE_URL =
  "https://impactlink-ai-platform-production.up.railway.app/api/projects";
...
const Typewriter = ({ text }) => {
  ...
  return (
    <p className="text-gray-700 mb-2 font-mono text-xs leading-relaxed">
      {displayedText}
    </p>
  );
};
...
const Dashboard = () => {
  ...
  const runScan = (latInput, lngInput) => {
    ...
    setTimeout(() => {
      ...
      axios
        .post(`${API_BASE_URL}/check-conflict`, inputPayload)
        .then((res) => {
          setDuplicates(res.data);

          if (res.data.length > 0) {
            generateAiReport(res.data[0].projectName);
          }
        })
        .catch((err) => {
      ...
      <div className="flex items-center justify-center h-screen bg-gray-900 font-sans text-white">
        <div className="bg-gray-800 p-10 rounded-2xl shadow-2xl w-full max-w-2xl border border-gray-700">
          ...
          <div className="space-y-5">
            ...
            <button
              onClick={() => runScan(formData.lat, formData.lng)}
              className="w-full mt-6 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold py-4 rounded-lg text-lg shadow-lg"
            >
              🚀 Initialize Scan
            </button>
          </div>
  ...
  if (view === "scanning") {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-black text-blue-400">
        ...
        <h2 className="text-3xl font-mono animate-pulse mt-8">
          SCANNING ECOSYSTEM...
        </h2>
      </div>
    );
    ...
    <div className="flex h-screen w-screen overflow-hidden font-sans bg-gray-100">
      ...
      <div className="flex-grow h-full relative z-0">
        ...
        >
          ...
          {/* Conflict Line */}
          {duplicates.length > 0 &&
            projects.find((p) => p.name === duplicates[0].projectName) && (
              <>
                <Circle
                  center={[parseFloat(formData.lat), parseFloat(formData.lng)]}
                  radius={1000}
                ...
                <Polyline
                  positions={[
                    ...
                    [
                      projects.find(
                        (p) => p.name === duplicates[0].projectName
                      ).latitude,
                      projects.find(
                        (p) => p.name === duplicates[0].projectName
                      ).longitude,
          ...
          <div>
            <div className="bg-red-50 border border-red-200 p-4 rounded-xl mb-6">
              ...
              <p className="text-xs text-red-600">
                Similar initiative found nearby.
              </p>
            </div>
            ...
            <div className="space-y-4">
              {duplicates.map((partner, idx) => (
                <div
                  key={idx}
                  className="bg-white p-5 rounded-xl shadow-lg border border-gray-100 ring-1 ring-black/5"
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="bg-red-100 text-red-800 text-xs font-bold px-2 py-1 rounded-full">
                      {partner.duplicationScore}% Match
                    </span>
                  </div>
                  ...
                  <div className="mt-3 bg-purple-50 p-3 rounded-lg border border-purple-200 animate-fade-in">
                    ...
                    {loadingAi ? (
                      ...
                      aiInsight &&
                      aiInsight.partner === partner.projectName && (
                        <>
